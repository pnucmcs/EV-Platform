# Reservation Service DevOps (Helm)

Kubernetes assets for building and deploying the reservation-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/reservationservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/reservationservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/reservationservice:<tag> .
docker push ghcr.io/your-org/reservationservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install reservationservice devops/helm/reservationservice   -f devops/helm/reservationservice/values.yaml
```
