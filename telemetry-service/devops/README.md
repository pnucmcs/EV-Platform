# Telemetry Service DevOps (Helm)

Kubernetes assets for building and deploying the telemetry-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/telemetryservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/telemetryservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/telemetryservice:<tag> .
docker push ghcr.io/your-org/telemetryservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install telemetryservice devops/helm/telemetryservice   -f devops/helm/telemetryservice/values.yaml
```
