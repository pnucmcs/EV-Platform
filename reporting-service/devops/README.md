# Reporting Service DevOps (Helm)

Kubernetes assets for building and deploying the reporting-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/reportingservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/reportingservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/reportingservice:<tag> .
docker push ghcr.io/your-org/reportingservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install reportingservice devops/helm/reportingservice   -f devops/helm/reportingservice/values.yaml
```
