# User Service DevOps (Helm)

Kubernetes assets for building and deploying the user-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/userservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/userservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/userservice:<tag> .
docker push ghcr.io/your-org/userservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install userservice devops/helm/userservice   -f devops/helm/userservice/values.yaml
```
