# Station Service DevOps (Helm)

Kubernetes assets for building and deploying the station-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/stationservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/stationservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/stationservice:<tag> .
docker push ghcr.io/your-org/stationservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install stationservice devops/helm/stationservice   -f devops/helm/stationservice/values.yaml
```
