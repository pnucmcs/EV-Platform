# Notification Service DevOps (Helm)

Kubernetes assets for building and deploying the notification-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/notificationservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/notificationservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/notificationservice:<tag> .
docker push ghcr.io/your-org/notificationservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install notificationservice devops/helm/notificationservice   -f devops/helm/notificationservice/values.yaml
```
