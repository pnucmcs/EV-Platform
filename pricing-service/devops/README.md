# Pricing Service DevOps (Helm)

Kubernetes assets for building and deploying the pricing-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/pricingservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/pricingservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/pricingservice:<tag> .
docker push ghcr.io/your-org/pricingservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install pricingservice devops/helm/pricingservice   -f devops/helm/pricingservice/values.yaml
```
