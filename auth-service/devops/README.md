# Auth Service DevOps (Helm)

Kubernetes assets for building and deploying the auth-service with Helm.

## Layout
- `devops/Dockerfile` – multi-stage .NET 8 build/publish image.
- `devops/helm/authservice` – Helm chart (Deployment/Service/Secret/ServiceAccount).
- `devops/helm/authservice/values.yaml` – deployment values (dev defaults, sample settings).

## Build & push the container
```sh
docker build -f devops/Dockerfile -t ghcr.io/your-org/authservice:<tag> .
docker push ghcr.io/your-org/authservice:<tag>
```
Update the `image.repository`/`image.tag` in the values file to match what you push.

## Deploy with Helm
```sh
helm upgrade --install authservice devops/helm/authservice   -f devops/helm/authservice/values.yaml
```
